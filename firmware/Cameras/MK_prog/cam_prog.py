...
sensor.reset()
print("op")
#sensor.set_windowing((0,0,150, 150))
...
pin5 = Pin('P8', Pin.OUT_PP, Pin.PULL_NONE)
pin6 = Pin('P7', Pin.OUT_PP, Pin.PULL_NONE)
...
while(True):
    ...
    if(maxBlob):
        ...
        u = img.find_template(templateU, 0.6,roi = (0,0,22,22), steps = 2, serach = image.SEARCH_EX)
        ...
        u2 = img.find_template(templateU2, 0.6,roi = (0,0,22,22), steps = 2, serach = image.SEARCH_EX)
        ...
        us = img.find_template(templateUS, 0.6,roi = (0,0,22,22), steps = 2, serach = image.SEARCH_EX)
        if(h or h2 or hs):
            print("h")
            #img.draw_rectangle(h,fill = True, color = 123)
            pin5.value(1)
            pin6.value(1)
            LED(2).on()
        else:
            if(u or u2 or us):
                print("u")
                #img.draw_rectangle(u,fill = True, color = 123)
                pin5.value(1)
                pin6.value(0)
                LED(2).on()
            else:
                if(s or s2 or ss):
                    print("s")
                    #img.draw_rectangle(s,fill = True, color = 123)
                    pin5.value(0)
                    pin6.value(1)
                    LED(2).on()
                else:
