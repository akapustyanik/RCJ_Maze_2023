...
int8_t toNewPoint() {
  ...
  if (!lastPitch) {
    ...
    if (lasers[0] > dist_for[0] * 2.5 and !getWall(xRobot, yRobot, zRobot, orient) and !getSearch(xRobot, yRobot, zRobot, orient, 1)) { //ЕСЛИ СЧИТАЕТ ЧТО СТЕНКА - ПРОЕЗД - 2.5 ЗАМЕНИТЬ НА 3
    ...
    if (lasers[5] > dist_back[0] * 2.5 and !getWall(xRobot, yRobot, zRobot, orient + 2) and !getSearch(xRobot, yRobot, zRobot, orient + 2, 1)) {
  ...
  setMass(1, xRobot, yRobot, zRobot, orient, 0);
  queueI = preobrI(queueI + 1);
  while (bfsFlag) {
    if (nowI != queueI) {
      for (int i = 0; i < 4; i++) { //просмотр 4 направлений вокруг клетки
        if (getWall(queue[nowI][0], queue[nowI][1], queue[nowI][2], i) and !getMass(queue[nowI][0], queue[nowI][1], queue[nowI][2], i, 1) and bfsFlag) {
          // если между клетками в данном направлении нет стены и эта клетка еще на просматривалась
          queue[queueI][0] = preobrX(queue[nowI][0] - (i % 2) * (i - 2)); //записать координаты клетки в очередь
          ...
          // установить массу данной вершины графа (клетки)
          if (!getSearch(queue[queueI][0], queue[queueI][1], queue[queueI][2], i, 0)) { //если стены между клетками нет, но мы ее уже посещали
            target[0] = queue[queueI][0]; //установить эту клетку в качестве целевой точки
