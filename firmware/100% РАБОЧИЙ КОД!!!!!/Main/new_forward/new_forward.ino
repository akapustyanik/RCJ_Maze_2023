...
#define SCREEN_HEIGHT 128 //высота дисплея

// You can use any (4 or) 5 pins

#define SCLK_PIN 51 //протокол дисплея
#define MOSI_PIN 52
#define DC_PIN 50
#define CS_PIN 53
#define RST_PIN 49
...
// Color definitions
#define BLACK 0x0000 //базовые цвета диспплея
#define BLUE 0x001F
#define RED 0xF800
#define GREEN 0x07E0
#define CYAN 0x07FF
...
#define CATBUT1 47 //пины кнопок-лапок
#define CATBUT2 48

#include <SPI.h> //инклуды библы
#include <Adafruit_GFX.h>
#include <Adafruit_SSD1351.h>

Adafruit_SSD1351 tft = Adafruit_SSD1351(SCREEN_WIDTH, SCREEN_HEIGHT, CS_PIN, DC_PIN, MOSI_PIN, SCLK_PIN, RST_PIN); //объект класса дисплей

const unsigned char paw[] PROGMEM = { //рисунок лапки
...
const unsigned char ears[] PROGMEM = { //уши
  // 'ears', 42x10px
...
const unsigned char paw2[] PROGMEM = { //еще лапа
  // 'cat paw2', 32x32px
...
const unsigned char cat[] PROGMEM = { //кот
  //cat 64x128
...
void setTextSize(uint8_t ts) { //выставляет размер шрифта
  //СМОТРИ КОД ДЛЯ ДИСПЛЕЯ!!! ТУТ ДОЛЖНА БЫТЬ ПЕРЕМЕННАЯ!!! И ВООБЩЕ ТУТ НЕ ВСЕ ФУНКЦИИ И ФУНКЦИОНАЛ!!!
  tft.setTextSize(ts);
}

#define sgn(x) ((x) < 0 ? -1 : ((x) > 0 ? 1 : 0)) //знак числа
#define ASCII_CONVERT '0' //кодирование в аски

int16_t squareEnc = 950, l1Dist = 15, l2Dist = 120, lateralDist = 130, deltaEnc = 80, maxHorizont = 100, maxCorrEncs = 150; //дистанции для лазеров. 
//      энкодер клетки    не трогать, побочные перем,бок. расст. до ст, доп энк горка, энк горка,               
int16_t vMinTurn = 150, vForward = 1200;
// скорость поворота и проезда, не менять
float kGyroGlobal = 40, kLaser1Global = 6, kLaser2Global = 0, kViravnLaser = 7, kViravnTurn = 40, kViravnForward = 2;
//коэфы регулятора, не менять
int16_t dist_for[] = { 72, 420, 730, 960, 1030 }, dist_back[] = { 120, 410, 730, 910, 980 };
// массив значений показаний лазеров от клетки
void forward(int16_t v = vForward, int16_t distEnc = squareEnc, float kGyro = kGyroGlobal, float kLaser1 = kLaser1Global, float kLaser2 = kLaser2Global, float kViravn1 = kViravnForward);
...
int16_t blackColor[2] = { 575, 900 };
int16_t blueColor[2] = { 100, 200 };
int16_t silverColor[2] = { 10, 30 };
//цвета клеток
...
int8_t helps[6] = { 1, 0, 2, 0, 1, 2 };
int8_t numHelps = 0;
const int8_t numHelpsMax = 12;
int8_t nowHelps[4] = { 0, 0, 0, 0 };

int16_t lasers[] = { 0, 0, 0, 0, 0, 0 };
